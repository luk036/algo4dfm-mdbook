...
- Calculate all the areas of triangles with different _corners_.
- Problems:
...
- Problem: cannot handle _correlation_ between variables.
...
- Features of AA:
  - Handle correlation of variations by sharing _noise symbols_.
...
- An uncertain quantity is represented in an affine form (AAF):
  ...
  $$
  where
  - $\varepsilon_i \in [-1, 1]$ is called _noise symbol_.
- Exact results for affine operations ($\hat{a}+\hat{b}$,
  $\hat{a}-\hat{b}$ and $\alpha \cdot \hat{a}$)
- Results of non-affine operations (such as $\hat{a} \cdot \hat{b}$, $\hat{a}/\hat{b}$, $\max(\hat{a}, \hat{b}), \log(\hat{a})$) are _approximated_ in an affine form.
...
- For each ellipsoid iteration, $f(x_c,q)$ is obtained by _approximating_ $f(x_c,\hat{q})$ in an affine form:
